...
"""
build_intc_model.py
Builds Intel_IB_Model.xlsx — 11-tab, segment-driven 3-statement DCF model
...
def build_assumptions(wb, R):
    ...
    _arow_const(ws, r, "Risk-Free Rate (Rf)", 3.88, 4.25, 4.50, 0.045, "0.00%",
                "US 10Y Treasury, May 2026"); R["asp_rf"] = r; r += 1
    _arow_const(ws, r, "Equity Risk Premium (ERP)", 5.50, 5.50, 5.50, 0.055, "0.00%",
    ...
    _arow_const(ws, r, "Pre-Tax Cost of Debt (Kd)", 4.80, 5.00, 5.00, 0.050, "0.00%",
                "Weighted avg coupon on outstanding notes"); R["asp_kd"] = r; r += 1
    _arow(ws, r, "Target D/(D+E)", 30.0, 32.0, 32.0, 0.25, 0.20, 0.35, "0.0%",
    ...
    for seg_key, seg_label, h2024, h2025, yrs in [
        ...
        for yr_label, base, bull, bear, note_txt in yrs:
            ...
            _arow(ws, r, label, None, h2024, h2025, base/100.0, bull/100.0, bear/100.0, "0.0%", note_txt)
    ...
    for yr_label, base_gm, bull_gm, bear_gm, note_txt in [
        ...
        _arow(ws, r, f"  COGS as % of Revenue {yr_label}", 60.0, 67.3, 65.2,
              100.0 - base_gm, 100.0 - bull_gm, 100.0 - bear_gm, "0.0%",
    ...
    for yr_label, base, bull, bear, note_txt in [
        ...
        _arow(ws, r, f"  R&D % of Revenue {yr_label}", 29.6, 31.2, 26.1, base/100.0, bull/100.0, bear/100.0, "0.0%", note_txt)
    ...
    for yr_label, base, bull, bear, note_txt in [
        ...
        _arow(ws, r, f"  MG&A % of Revenue {yr_label}", 10.4, 10.4, 8.7, base/100.0, bull/100.0, bear/100.0, "0.0%", note_txt)
    ...
    _arow_const(ws, r, "Accrued Comp % of Revenue", 6.7, 6.3, 7.6, 7.0, "0.0%", "Midpoint of historical range"); R["asp_accrued_comp"] = r; r += 1
    _arow_const(ws, r, "Other CA % of Revenue", None, None, None, 15.0, "0.0%", "Includes ~$7.5B refundable tax credits"); R["asp_other_ca"] = r; r += 1
    _arow_const(ws, r, "Other CL % of Revenue", None, None, None, 25.0, "0.0%", "Stable relationship"); R["asp_other_cl"] = r; r += 1
    _arow_const(ws, r, "Income Tax Payable % of Revenue", None, None, None, 2.0, "0.0%", "Normalised level"); R["asp_tax_payable"] = r; r += 1
    ...
    _arow(ws, r, "Tax Rate — FY2026E", -119.8, -71.6, 98.3, 12.0, 11.0, 15.0, "0.0%",
          "Anchored to Q2 2026 guided 11%. ETR=Effective Tax Rate for tax shield calc")
    R["asp_etr_sel"] = r; r += 1
    # Tax rate for each year
    _arow(ws, r, "Tax Rate — FY2027E", None, None, None, 13.0, 12.0, 15.0, "0.0%",
          "Valuation allowance means US losses provide no benefit")
    R["asp_etr_2027e"] = r; r += 1
    _arow(ws, r, "Tax Rate — FY2028E", None, None, None, 13.0, 12.0, 15.0, "0.0%",
...
# ============================================================
# TAB 4: SEGMENT_REVENUE (Bottom-Up Build)
# ============================================================
...
def build_segment_revenue(wb, R):
    ...
    # External-facing segments (CCG, DCAI, All Other) + Foundry (total, mostly internal)
    for seg_key, seg_label, hist_rev, growth_keys in segs:
        ...
        prev_col = HIST_COLS[2]  # 2025A -> 2026E
        for fi, fc in enumerate(FCST_COLS):
            _fval(ws, r, fc,
                  f"={CL(prev_col)}{r}*(1+Assumptions!I{R[growth_keys[fi]]})")
            prev_col = fc
        _note(ws, r, f"Forecast: Prior year x (1 + Assumptions!Selected growth)")
...
def build_cover(wb):
    ...
    info = [
        ...
        ("Model Architecture", "11 tabs: Cover -> Key_Summary -> Assumptions -> Segment_Revenue -> Segment_PL -> Consolidated_PL -> BS -> Cash_Flow -> DCF -> Sensitivity -> Ratio_Analysis"),
        ("", ""),
...
def build_model():
    """Create the complete 11-tab workbook."""
    ...
    ws0 = wb.active
    ws0.title = "Cover"
    for name in ["Key_Summary", "Assumptions", "Segment_Revenue", "Segment_PL",
                 "Consolidated_PL", "BS", "Cash_Flow", "DCF", "Sensitivity", "Ratio_Analysis"]:
    ...
    print("  3/11 Segment_Revenue ...")
    build_segment_revenue(wb, R)

    print("  4/11 Segment_PL (pass 1) ...")
    build_segment_pl(wb, R)

    print("  5/11 Consolidated_PL ...")
    build_consolidated_pl(wb, R)

    print("  6/11 Segment_PL (pass 2 - cross-check) ...")
    build_segment_pl_pass2(wb, R)

    print("  7/11 Balance Sheet ...")
    build_bs(wb, R)

    print("  8/11 Cash_Flow ...")
    build_cash_flow(wb, R)

    print("  9/11 DCF ...")
    build_dcf(wb, R)

    print(" 10/11 Sensitivity ...")
    build_sensitivity(wb, R)

    print(" 11/11 Ratio_Analysis ...")
    build_ratio_analysis(wb, R)

    print("  Key_Summary (dashboard) ...")
    build_key_summary(wb, R)

    # Reorder sheets: Key_Summary should be #2 (after Cover)
    # build_key_summary was built last but we want it at position 2
    wb.move_sheet("Key_Summary", offset=-9)
...
def check_model(output_path=OUTPUT):
    ...
    expected_tabs = ["Cover", "Key_Summary", "Assumptions", "Segment_Revenue",
